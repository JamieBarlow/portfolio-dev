const projectData = {
  ...
  yelpCamp: {
    ...
    webStack: [
      ...
      "I used EJS to create page templates, with consistent 'partial' layout elements such as a navbar, footer and error messaging. The library also allowed me to embed or interpolate JS for dynamically updated content which could be linked to the user-specific authorization/permissions - for example, showing and hiding the option to leave a review depending on the user's login status, and the option to delete a campground or review only if they are the author of that particular campground or review.",
      ...
      "Data persistence and relationships were managed with MongoDB, a noSQL database, and Mongoose, which extends this with the ability to define consistent data schemas/models. While a 'relational' or SQL-based database would also have sufficed, MongoDB was chosen because of its frequent pairing and compatibility with both Express and Node.js, as well as flexible 'two-way referencing' between data models, such as campgrounds and reviews. This was then ported to MongoDB Atlas, a cloud-based equivalent.",
      "I implemented client-side validations using BootStrap, with user feedback 'toast' pop-up notifications, and some server-side data validations using <a href='https://mongoosejs.com/docs/guide.html'>Mongoose schemas</a>. For added security, this functionality was extended with the <a href='https://joi.dev/'>joi</a> library, which allowed me to include further server-side validations in case of any incoming data that bypasses the form submission layer, e.g. those made via external API requests.",
      "The course taught me some very useful fundamentals in terms of authenticating a user from scratch, but to simplify the process I used <a href='https://www.passportjs.org/'>Passport.js</a> middleware, which provides pre-defined 'strategies' for authenticating usernames/passwords, and <a href='https://www.npmjs.com/package/passport-local-mongoose'>passport-local-mongoose</a>, which adds useful methods to the Mongoose data model, while allowing sensitive user data to be stored in secure, encrypted (salted and hashed) form. The end result is a secure, robust registration and login process.",
    ...
    lessons: [
      ...
      "The project taught me how to effectively 'seed' my database for testing while in a production environment, which I needed to do multiple times throughout development as my app's data structure evolved. I used the <a href='https://unsplash.com/developers'>Unsplash Source API</a> to fetch a series of images from a collection, and populated the rest of the data with randomised location names, descriptors, and geocoding data.",
      'Security issues - salting/hashing passwords',
      "I learned about ways to make the data in HTTP requests 'stateful', using session and cookie data, which means allowing the app to remember the status of a logged in user and their permissions. This is achieved server-side using session data (with <a href='https://www.npmjs.com/package/express-session'>express-session</a>), and passed to the client for cookie-based authentication. To enable cookie signing and parsing, I used the <a href='https://www.npmjs.com/package/cookie-parser'>cookie-parser</a> middleware.",
